...
function writeConfigDefaults(configPath) {
  // check files exist
  if (!fse__default["default"].existsSync(configPath)) return; // this will be the *full* tsconfig.json with any extensions deeply merged

  let fullConfig = tsconfigLoader.loadTsconfig(configPath); // this will be the user's actual tsconfig file
  ...
  let configType = path__namespace.basename(configPath); // sanity checks to make sure we can write the compilerOptions
