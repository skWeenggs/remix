...
const ChatBoot=()=>{
    ...
    {state ?
      ...
      <div  className="bg-[#b92b2f] fixed block mt-5  right-0 top-[30%] hover:cursor-pointer h-[40%] max-w-[9%] rounded-r-lg scale-[-1] z-50" onClick={handleChange}>
        <h1 className=" text-white font-solid  md:m-2 text-lg 2xl:text-2xl " style={{ marginTop:'20px', 'writingMode': 'vertical-rl',display:'table-caption'}}> Send Massage</h1> 
     </div>
    
     :
     <div  className='z-50 h-[500px]  2xl:h-[600px] max-w-[320px] 2xl:max-w-[400px] bottom-0 right-0 fixed inline-grid'>
        ...
        <div className="p-5 text-sm  text-white 2xl:text-xl ">
         Please fill out the form below and we will get back to you as soon as possible.
        </div>
        </div>
         <div className=" overflow-auto bg-white p-5">
         <form className="w-full max-w-sm p-5 bg-[#fafafa] border-l-4 border-red-500 rounded"  onSubmit={handleSubmit}>
            
                <div className="relative z-0 mb-6 w-full  group text-xs ">
                    <input type="text"  className="block py-2.5 2xl:text-2xl px-0 w-full outline-1 text-sm text-gray-700 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-red-500 focus:outline-none focus:ring-0 focus:border-red-600 peer" placeholder=" " name={name} onChange={(e)=>{setName(e.target.value)}} />
                    <label for="name" class="absolute text-sm 2xl:text-xl  text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-red-600 peer-focus:dark:text-red-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">* Name</label>
                    <div style={{color:'red'}} id="name-error" class="error 2xl:text-xl"></div>
                </div>
                ...
                <div className="relative z-0 mb-6 w-full group text-xs">
                    ...
                    <div style={{color:'red'}} id="massage-error" className="error 2xl:text-xl"></div>
                </div>

            <div className="md:flex md:items-center">
               
                <button type="submit" className="inline-block px-6 py-2.5 w-full 2xl:text-xl bg-red-600 text-white font-medium text-sm  uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out"> <i className="fa fa-paper-plane rotate-45 m-2 "></i> Submit</button>
            </div>
            
        </form>
        </div>
        <div className="text-center bg-[#e5e5e5] z-50">
            <button className=" hover:bg-white text-xs w-full m-auto  my-1 rounded-full justify-center ">
                <a href="https://www.tawk.to/?utm_source=tawk-messenger&utm_medium=link&utm_campaign=referral&utm_term=5858baa75191003fde7bc865" className="2xl:text-2xl">
